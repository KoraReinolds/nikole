...
<template>
  <div
    ...
    <div
      ...
      <h1 class="font-m pt-4 text-6xl text-secondary">О нас</h1>
      ...
      <h5 class="font-bold text-2xl sm:text-3xl my-4">Вас ожидают:</h5>
      <ul class="flex flex-col gap-2">
        <li v-for="item in aboutList">
          <List class="w-5 h-5 mb-2 fill-secondary inline-block mr-2" />
          <a class="text-xl underline font-bold" :href="`#${item.ref}`">{{ item.title }}</a>
          {{ item.text }}
        </li>
      </ul>
      ...
      <p class="hidden sm:block">Бесплатный wi-fi<br>☕️Вкусный чай/кофе с конфетами</p>

      <p class="hidden sm:block">Приходите и мы позаботимся о Вашем теле, чтобы Вашей душе было приятно и драйвово в нем
        жить!</p>
